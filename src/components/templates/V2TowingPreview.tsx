...
const STOCK_GALLERY = [
  ...
  "https://images.unsplash.com/photo-1533473359331-2f2db1e9375c?w=600&q=80",
...
function AnimatedSection({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <motion.div initial={{ opacity: 0, scale: 0.92 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.5, ease: "easeOut" as const }} className={className}>
      {children}
    </motion.div>
  );
}
...
export default function V2TowingPreview({ data }: { data: GeneratedSiteData }) {
  ...
  const galleryImages = data.photos?.length > 3 ? data.photos.slice(2, 6) : pickGallery(STOCK_GALLERY, data.businessName);
