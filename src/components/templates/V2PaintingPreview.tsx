...
const STOCK_ABOUT = "https://images.unsplash.com/photo-1589939705384-5185137a7f0f?w=600&q=80";
const STOCK_GALLERY = [
...
function SplatterPattern({ opacity = 0.03, accent }: { opacity?: number; accent: string }) {
  const patternId = `splatterV2-${accent.replace("#", "")}`;
  return (
...
function HeroDecor({ accent }: { accent: string }) {
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 1200 800" preserveAspectRatio="none" style={{ opacity: 0.05 }}>
      <circle cx="200" cy="300" r="60" fill={accent} opacity="0.15" />
      <circle cx="250" cy="280" r="25" fill={PINK_ACCENT} opacity="0.12" />
      <circle cx="180" cy="340" r="15" fill={accent} opacity="0.2" />
      <circle cx="900" cy="500" r="50" fill={PINK_ACCENT} opacity="0.1" />
      <circle cx="950" cy="470" r="20" fill={accent} opacity="0.15" />
      <circle cx="600" cy="200" r="40" fill={accent} opacity="0.08" />
    </svg>
  );
}
...
export default function V2PaintingPreview({ data }: { data: GeneratedSiteData }) {
  ...
  const processSteps = [
    { step: "01", title: "Free Color Consult", desc: "We visit your space, discuss your vision, and help you choose the perfect colors and finishes." },
    { step: "02", title: "Detailed Estimate", desc: "Receive a transparent, itemized quote with no hidden fees. We explain every line item." },
    { step: "03", title: "Expert Painting", desc: "Our skilled painters prep, prime, and paint with meticulous attention to detail and clean edges." },
    { step: "04", title: "Final Walkthrough", desc: "We do a thorough inspection with you, touch up any details, and leave your space spotless." },
  ];

  const faqs = [
    { q: `What painting services does ${data.businessName} offer?`, a: `We offer a full range of painting services including ${data.services.slice(0, 3).map(s => s.name).join(", ")}, and more. Whether residential or commercial, we deliver flawless results.` },
  ...
  return (
    <main className="relative min-h-[100dvh] overflow-x-hidden" style={{ background: CHARCOAL, color: "#f1f5f9" }}>
      ...
      <section className="relative min-h-[100dvh] flex items-center pt-24 z-10 overflow-hidden">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] rounded-full blur-[160px] pointer-events-none" style={{ background: `${PINK_ACCENT}06` }} />
        <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6 w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          <div className="space-y-8">
            <div><p className="text-sm uppercase tracking-widest mb-4" style={{ color: ACCENT }}>Professional Painting Services</p><h1 className="text-3xl md:text-6xl tracking-tighter leading-none font-bold text-white" style={{ textShadow: "0 2px 10px rgba(0,0,0,0.5)" }}>{data.tagline}</h1></div>
            ...
            <div className="flex flex-wrap gap-6 text-sm text-slate-400"><span className="flex items-center gap-2"><MapPin size={16} weight="duotone" style={{ color: ACCENT }} /><MapLink address={data.address} /></span><span className="flex items-center gap-2"><Palette size={16} weight="duotone" style={{ color: ACCENT }} />Free Color Consultation</span></div>
          </div>
          <div className="hidden md:block relative"><div className="relative rounded-2xl overflow-hidden border border-white/10"><img src={heroImage} alt={`${data.businessName} painting work`} className="w-full h-[500px] object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-[#1a1a1a] via-transparent to-transparent" /><div className="absolute inset-0 bg-gradient-to-r from-[#1a1a1a]/40 to-transparent" /><div className="absolute bottom-6 left-6 flex items-center gap-3"><div className="px-4 py-2 rounded-full backdrop-blur-md bg-black/50 border flex items-center gap-2" style={{ borderColor: `${ACCENT}4d` }}><ShieldCheck size={18} weight="fill" style={{ color: ACCENT }} /><span className="text-sm font-semibold text-white">Licensed &amp; Insured</span></div></div></div></div>
        </div>
      </section>
      ...
      <section className="relative z-10 py-16 overflow-hidden border-y" style={{ borderColor: `${ACCENT}1a` }}>
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10"><div className="grid grid-cols-2 md:grid-cols-4 gap-8">{data.stats.map((stat, i) => { const statIcons = [PaintBrush, HouseLine, Star, Clock]; const Icon = statIcons[i % statIcons.length]; return (<div key={stat.label} className="text-center"><div className="flex items-center justify-center gap-2 mb-2"><Icon size={22} weight="fill" style={{ color: ACCENT }} /><span className="text-3xl md:text-4xl font-extrabold text-white">{stat.value}</span></div><span className="text-slate-500 text-sm font-medium tracking-wide uppercase">{stat.label}</span></div>); })}</div></div>
      </section>
      ...
      <section id="services" className="relative z-10 py-24 md:py-32 overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none"><div className="absolute top-[10%] right-[5%] w-[500px] h-[500px] rounded-full blur-[160px]" style={{ background: `${ACCENT}08` }} /><div className="absolute bottom-[10%] left-[5%] w-[400px] h-[400px] rounded-full blur-[140px]" style={{ background: `${PINK_ACCENT}05` }} /></div>
      ...
      <section id="about" className="relative z-10 py-24 md:py-32 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10"><div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="relative"><div className="rounded-2xl overflow-hidden border border-white/10"><img src={aboutImage} alt={`${data.businessName} team`} className="w-full h-[400px] object-cover" /></div><div className="absolute -bottom-4 -right-4 md:bottom-6 md:-right-6"><div className="px-5 py-3 rounded-xl backdrop-blur-md border text-white font-bold text-sm shadow-lg" style={{ background: `${ACCENT}e6`, borderColor: `${ACCENT}80` }}>{data.stats[0] ? `${data.stats[0].value} ${data.stats[0].label}` : "Color Experts"}</div></div></div>
          <div><span className="inline-block text-xs font-bold uppercase tracking-[0.25em] mb-4 px-4 py-1.5 rounded-full border" style={{ color: ACCENT, borderColor: `${ACCENT}33`, background: `${ACCENT}0d` }}>Our Story</span><h2 className="text-3xl md:text-5xl font-extrabold tracking-tight mb-6 text-white">Transforming Spaces with Color</h2><p className="text-slate-400 leading-relaxed mb-8">{data.about}</p><div className="grid grid-cols-2 gap-4">{[{ icon: ShieldCheck, label: "Licensed & Insured" }, { icon: Palette, label: "Color Experts" }, { icon: Star, label: "5-Star Rated" }, { icon: CheckCircle, label: "Satisfaction Guaranteed" }].map((badge) => (<GlassCard key={badge.label} className="p-4 flex items-center gap-3"><div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: ACCENT_GLOW }}><badge.icon size={20} weight="duotone" style={{ color: ACCENT }} /></div><span className="text-sm font-semibold text-white">{badge.label}</span></GlassCard>))}</div></div>
        </div></div>
      </section>
      ...
      <section id="gallery" className="relative z-10 py-24 md:py-32 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          <SectionHeader badge="Our Portfolio" title="Recent Transformations" subtitle="See the stunning before-and-after results of our painting projects." accent={ACCENT} />
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {galleryImages.map((src, i) => { const captions = ["Living Room Refresh", "Exterior Makeover", "Kitchen Cabinet Painting", "Bedroom Color Update", "Commercial Office", "Accent Wall Design"]; return (<div key={i} className="group relative rounded-2xl overflow-hidden border border-white/[0.06] hover:border-opacity-30 transition-all duration-500 aspect-square"><img src={src} alt={captions[i] || `Project ${i + 1}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" /><div className="absolute bottom-0 left-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500"><h3 className="text-sm font-bold text-white">{captions[i] || `Project ${i + 1}`}</h3></div></div>); })}
          </div>
        </div>
      </section>
      ...
      <section className="relative z-10 py-20 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
          ...
          <p className="text-lg text-white/80 mb-8 max-w-xl mx-auto">Get a free color consultation and detailed estimate. Let us bring your vision to life with expert painting.</p>
      ...
      <section className="relative z-10 py-24 md:py-32 overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-6 relative z-10"><SectionHeader badge="FAQ" title="Common Questions" accent={ACCENT} /><div className="space-y-3">{faqs.map((faq, i) => <AccordionItem key={i} question={faq.q} answer={faq.a} isOpen={openFaq === i} onToggle={() => setOpenFaq(openFaq === i ? null : i)} />)}</div></div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10"><div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <GlassCard className="p-8">
            ...
            <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
              ...
              <div><label className="block text-sm text-slate-400 mb-1.5">Service Needed</label><select className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none transition-colors text-sm"><option value="" className="bg-neutral-900">Select a service</option>{data.services.map((s) => <option key={s.name} value={s.name.toLowerCase().replace(/\s+/g, "-")} className="bg-neutral-900">{s.name}</option>)}</select></div>
              <div><label className="block text-sm text-slate-400 mb-1.5">Project Details</label><textarea rows={3} className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-500 focus:outline-none transition-colors text-sm resize-none" placeholder="Describe your painting project..." /></div>
      ...
      <section className="relative z-10 py-16 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 text-center"><ShimmerBorder accent={ACCENT}><div className="p-8 md:p-12"><ShieldCheck size={48} weight="fill" style={{ color: ACCENT }} className="mx-auto mb-4" /><h2 className="text-2xl md:text-4xl font-extrabold text-white mb-4">Our Quality Guarantee</h2><p className="text-slate-400 leading-relaxed max-w-2xl mx-auto text-lg">Every project by {data.businessName} is backed by our satisfaction guarantee. We use premium paints, proper preparation, and meticulous technique to deliver results that last.</p><div className="flex flex-wrap justify-center gap-4 mt-8">{["Premium Paints", "Clean Edges", "No Hidden Fees", "Satisfaction Guaranteed"].map((item) => <span key={item} className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium border" style={{ color: ACCENT, borderColor: `${ACCENT}33`, background: `${ACCENT}0d` }}><CheckCircle size={16} weight="fill" /> {item}</span>)}</div></div></ShimmerBorder></div>
      </section>
      ...
      <footer className="relative z-10 border-t border-white/5 py-10 overflow-hidden">
        ...
        <div className="mx-auto max-w-6xl px-6 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div><div className="flex items-center gap-2 mb-3"><PaintBrush size={22} weight="fill" style={{ color: ACCENT }} /><span className="text-lg font-bold text-white">{data.businessName}</span></div><p className="text-sm text-slate-500 leading-relaxed">{data.about.length > 120 ? data.about.slice(0, 120).trim() + "..." : data.about}</p></div>
            <div><h4 className="text-sm font-semibold text-white mb-3">Quick Links</h4><div className="space-y-2">{["Services", "About", "Gallery", "Contact"].map((link) => <a key={link} href={`#${link.toLowerCase()}`} className="block text-sm text-slate-500 hover:text-white transition-colors">{link}</a>)}</div></div>
          ...
          <div className="border-t border-white/5 pt-6 flex flex-col md:flex-row items-center justify-between gap-4"><div className="flex items-center gap-2 text-sm text-slate-500"><PaintBrush size={14} weight="fill" style={{ color: ACCENT }} /><span>{data.businessName} &copy; {new Date().getFullYear()}</span></div><div className="flex items-center gap-2 text-xs text-slate-600"><BluejayLogo className="w-4 h-4" /><span>Website created by Bluejay Business Solutions</span></div></div>
        </div>
      </footer>
