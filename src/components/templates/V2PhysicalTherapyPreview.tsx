...
const STOCK_HERO = "https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=1400&q=80";
const STOCK_ABOUT = "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=600&q=80";
...
export default function V2PhysicalTherapyPreview({ data }: { data: GeneratedSiteData }) {
  ...
  return (
    <main className="relative min-h-[100dvh] overflow-x-hidden" style={{ background: BG, color: "#1e293b" }}>
      ...
      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50"><div className="mx-auto max-w-7xl px-4 md:px-6 py-4"><GlassCard className="flex items-center justify-between px-4 md:px-6 py-3"><div className="flex items-center gap-2"><Heartbeat size={24} weight="fill" style={{ color: ACCENT }} /><span className="text-lg font-bold tracking-tight text-slate-900">{data.businessName}</span></div><div className="hidden md:flex items-center gap-8 text-sm text-slate-600"><a href="#services" className="hover:text-slate-900 transition-colors">Services</a><a href="#about" className="hover:text-slate-900 transition-colors">About</a><a href="#conditions" className="hover:text-slate-900 transition-colors">Conditions</a><a href="#contact" className="hover:text-slate-900 transition-colors">Contact</a></div><div className="flex items-center gap-3"><MagneticButton className="px-4 md:px-5 py-2 rounded-full text-sm font-semibold text-slate-900 transition-colors cursor-pointer" style={{ background: ACCENT } as React.CSSProperties}>Book Now</MagneticButton><button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden p-2 rounded-lg text-slate-900 hover:bg-slate-100 transition-colors">{mobileMenuOpen ? <X size={24} /> : <List size={24} />}</button></div></GlassCard>
      ...
      <section className="relative min-h-[100dvh] flex items-center pt-24 z-10 overflow-hidden">
        ...
        <WavePattern opacity={0.05} accent={ACCENT} />
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6 w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          <div className="space-y-8">
            <div><p className="text-sm uppercase tracking-widest mb-4" style={{ color: GREEN }}>Evidence-Based Physical Therapy</p><h1 className="text-3xl md:text-6xl tracking-tighter leading-none font-bold text-slate-900" style={{ textShadow: "none" }}>{data.tagline}</h1></div>
            ...
            <div className="flex flex-wrap gap-4">
              <MagneticButton className="px-8 py-4 rounded-full text-base font-semibold text-slate-900 flex items-center gap-2 cursor-pointer" style={{ background: ACCENT } as React.CSSProperties}>Schedule Evaluation <ArrowRight size={18} weight="bold" /></MagneticButton>
      ...
      <section className="relative z-10 py-16 overflow-hidden border-y" style={{ borderColor: `${ACCENT}1a` }}>
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10"><div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {data.stats.map((stat, i) => { const icons = [Heartbeat, Clock, CheckCircle, Star]; const Icon = icons[i % icons.length]; return <div key={stat.label} className="text-center"><div className="flex items-center justify-center gap-2 mb-2"><Icon size={22} weight="fill" style={{ color: ACCENT }} /><span className="text-3xl md:text-4xl font-extrabold text-slate-900">{stat.value}</span></div><span className="text-slate-500 text-sm font-medium tracking-wide uppercase">{stat.label}</span></div>; })}
        </div></div>
      </section>
      ...
      <section id="about" className="relative z-10 py-24 md:py-32 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10"><div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="relative"><div className="rounded-2xl overflow-hidden border border-slate-200"><img src={aboutImage} alt={`${data.businessName} clinic`} className="w-full h-[400px] object-cover" /></div><div className="absolute -bottom-4 -right-4 md:bottom-6 md:-right-6"><div className="px-5 py-3 rounded-xl backdrop-blur-md border text-slate-900 font-bold text-sm shadow-lg" style={{ background: `${ACCENT}e6`, borderColor: `${ACCENT}80` }}>{data.stats[0] ? `${data.stats[0].value} ${data.stats[0].label}` : "Expert Care"}</div></div></div>
          <div><span className="inline-block text-xs font-bold uppercase tracking-[0.25em] mb-4 px-4 py-1.5 rounded-full border" style={{ color: ACCENT, borderColor: `${ACCENT}33`, background: `${ACCENT}0d` }}>About Us</span><h2 className="text-3xl md:text-5xl font-extrabold tracking-tight mb-6 text-slate-900">Your Recovery, Our Mission</h2><p className="text-slate-600 leading-relaxed mb-8">{data.about}</p>
      ...
      <section id="conditions" className="relative z-10 py-24 md:py-32 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">{conditions.map((c) => <GlassCard key={c} className="p-4 flex items-center gap-3"><CheckCircle size={18} weight="fill" style={{ color: GREEN }} /><span className="text-sm font-medium text-slate-900">{c}</span></GlassCard>)}</div>
        </div>
      </section>
      ...
      <section id="contact" className="relative z-10 py-24 md:py-32 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10"><div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <GlassCard className="p-8"><h3 className="text-xl font-semibold text-slate-900 mb-6">Request an Appointment</h3><form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label className="block text-sm text-slate-600 mb-1.5">First Name</label><input type="text" className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400 focus:outline-none text-sm" placeholder="John" /></div><div><label className="block text-sm text-slate-600 mb-1.5">Last Name</label><input type="text" className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400 focus:outline-none text-sm" placeholder="Doe" /></div></div>
