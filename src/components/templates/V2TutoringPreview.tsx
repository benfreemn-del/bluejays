...
import { BookOpen, GraduationCap, Student, Clock, Phone, MapPin, CaretDown, List, X, Star, ArrowRight, CheckCircle, ShieldCheck, Lightbulb, ChartLineUp } from "@phosphor-icons/react";
...
const TEXT_PRIMARY = "#1c1007";
const TEXT_SECONDARY = "#57534e";
const TEXT_MUTED = "#a8a29e";
const CARD_BORDER = "#fde68a";

function getAccent(c?: string) { const a = c || DEFAULT_ACCENT; return { ACCENT: a, ACCENT_GLOW: `${a}15` }; }

// eslint-disable-next-line @typescript-eslint/no-explicit-any
const ICON_MAP: Record<string, any> = { math: ChartLineUp, read: BookOpen, writ: BookOpen, sci: Lightbulb, test: GraduationCap, sat: GraduationCap, act: GraduationCap, essay: BookOpen, english: BookOpen, history: BookOpen, college: GraduationCap, study: Lightbulb, execut: Lightbulb };
function getServiceIcon(n: string) { const l = n.toLowerCase(); for (const [k, I] of Object.entries(ICON_MAP)) { if (l.includes(k)) return I; } return Student; }

const STOCK_HERO = "https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=1400&q=80";
const STOCK_ABOUT = "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=600&q=80";
const STOCK_GALLERY = [
...
function ShimmerBorder({ children, accent }: { children: React.ReactNode; accent: string }) {
  return (
    <div className="relative rounded-2xl p-[1px] overflow-hidden">
      <motion.div className="absolute inset-0 rounded-2xl" style={{ background: `conic-gradient(from 0deg, transparent, ${accent}, transparent, #fcd34d, transparent)` }} animate={{ rotate: [0, 360] }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }} />
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50">
      <div className="bg-white/90 backdrop-blur-sm border-t px-4 py-2 flex items-center justify-center gap-4" style={{ borderColor: CARD_BORDER }}>
        <p className="text-xs" style={{ color: TEXT_MUTED }}><span className="inline-block w-2 h-2 rounded-full bg-amber-500 mr-1.5 animate-pulse" />47 businesses in your area upgraded their website this month</p>
...
export default function V2TutoringPreview({ data }: { data: GeneratedSiteData }) {
  ...
  const processSteps = [
    { step: "01", title: "Free Consultation", desc: "We meet with you and your student to understand their goals, learning style, and areas that need the most attention." },
    { step: "02", title: "Personalized Plan", desc: "Our tutors design a customized curriculum aligned with your student's school curriculum and academic goals." },
    { step: "03", title: "Engaging Sessions", desc: "One-on-one sessions in-person or online, using proven methods to build understanding and confidence." },
    { step: "04", title: "Track Progress", desc: "We provide regular progress updates and adjust our approach to ensure your student keeps improving." },
  ];

  const faqs = [
    { q: "What subjects and grade levels do you tutor?", a: `${data.businessName} offers tutoring in ${data.services.slice(0, 3).map((s: { name: string }) => s.name).join(", ")}, and more. We work with students from elementary school through college.` },
    { q: "Do you offer in-person and online tutoring?", a: `Yes! ${data.businessName} offers both in-person sessions at our learning center and convenient online tutoring via video call. We will find the format that works best for your student.` },
    { q: "How often should my child meet with a tutor?", a: "Most students benefit from 1-2 sessions per week. For test prep or students who need intensive support, more frequent sessions may be recommended. We will tailor the schedule to your needs." },
    { q: "How quickly will I see results?", a: "Many students see improvement within the first few sessions. Consistent effort and regular attendance are key. We set measurable goals so you can track progress along the way." },
  ];
  ...
  return (
    <main className="relative min-h-[100dvh] overflow-x-hidden" style={{ background: BG, color: TEXT_PRIMARY }}>
      <nav className="fixed top-0 left-0 right-0 z-50">
        <div className="mx-auto max-w-7xl px-4 py-4">
          <div className="flex items-center justify-between px-5 py-3 rounded-2xl bg-white/85 backdrop-blur-lg border shadow-sm" style={{ borderColor: CARD_BORDER }}>
            <div className="flex items-center gap-2"><GraduationCap size={24} weight="fill" style={{ color: ACCENT }} /><span className="text-lg font-bold" style={{ color: TEXT_PRIMARY }}>{data.businessName}</span></div>
            <div className="hidden md:flex items-center gap-8 text-sm" style={{ color: TEXT_SECONDARY }}>
              {["Services", "About", "Gallery", "Contact"].map(l => <a key={l} href={`#${l.toLowerCase()}`} className="hover:text-amber-700 transition-colors">{l}</a>)}
            </div>
            <div className="flex items-center gap-3">
              <MagneticButton className="px-5 py-2 rounded-full text-sm font-semibold text-white cursor-pointer" style={{ background: ACCENT } as React.CSSProperties}>Free Consultation</MagneticButton>
            ...
            {mobileMenuOpen && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="md:hidden mt-2 overflow-hidden">
                <div className="flex flex-col gap-1 px-4 py-4 rounded-2xl bg-white border shadow-sm" style={{ borderColor: CARD_BORDER }}>
                  {["Services", "About", "Gallery", "Contact"].map(l => <a key={l} href={`#${l.toLowerCase()}`} onClick={() => setMobileMenuOpen(false)} className="block px-4 py-3 rounded-lg text-sm hover:bg-stone-50" style={{ color: TEXT_SECONDARY }}>{l}</a>)}
                </div>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <section className="relative min-h-[100dvh] flex items-center pt-24 z-10 overflow-hidden">
        <div className="absolute inset-0" style={{ background: "linear-gradient(135deg, #fffbeb 0%, #fffbf5 50%, #fef3e2 100%)" }} />
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6 w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            <div>
              <p className="text-sm uppercase tracking-widest mb-4 font-semibold" style={{ color: ACCENT }}>Expert Academic Tutoring</p>
            ...
            <div className="flex flex-wrap gap-4">
              <MagneticButton className="px-8 py-4 rounded-full text-base font-semibold text-white flex items-center gap-2 cursor-pointer shadow-lg" style={{ background: ACCENT } as React.CSSProperties}>Get Started Today <ArrowRight size={18} weight="bold" /></MagneticButton>
            ...
            <div className="flex flex-wrap gap-6 text-sm" style={{ color: TEXT_SECONDARY }}>
              ...
              <span className="flex items-center gap-2"><ShieldCheck size={16} weight="duotone" style={{ color: ACCENT }} />In-Person & Online</span>
            </div>
          </div>
            ...
            <div className="rounded-2xl overflow-hidden border shadow-xl" style={{ borderColor: CARD_BORDER }}>
              ...
              <div className="absolute bottom-6 left-6">
                <div className="px-4 py-2 rounded-full backdrop-blur-md bg-white/90 border flex items-center gap-2 shadow-sm" style={{ borderColor: CARD_BORDER }}>
                  <GraduationCap size={18} weight="fill" style={{ color: ACCENT }} />
                  <span className="text-sm font-semibold" style={{ color: TEXT_PRIMARY }}>Proven Results</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {data.stats.map((stat: { label: string; value: string }, i: number) => {
              const icons = [Student, ChartLineUp, Star, GraduationCap];
      ...
      <section id="services" className="relative z-10 py-24 md:py-32" style={{ background: BG }}>
        <div className="max-w-6xl mx-auto px-6">
          <SectionHeader badge="What We Teach" title="Tutoring Programs" subtitle={`${data.businessName} offers personalized tutoring for students of all ages and skill levels.`} accent={ACCENT} />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="relative">
              <div className="rounded-2xl overflow-hidden border shadow-lg" style={{ borderColor: CARD_BORDER }}>
                <img src={aboutImage} alt={`${data.businessName} tutors`} className="w-full h-[400px] object-cover" />
              </div>
              <div className="absolute -bottom-4 -right-4 md:bottom-6 md:-right-6">
                <div className="px-5 py-3 rounded-xl text-white font-bold text-sm shadow-lg" style={{ background: ACCENT }}>{data.stats[0] ? `${data.stats[0].value} ${data.stats[0].label}` : "Certified Tutors"}</div>
              </div>
            </div>
            <div>
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight mb-6" style={{ color: TEXT_PRIMARY }}>Helping Students Reach Their Full Potential</h2>
              <p className="leading-relaxed mb-8" style={{ color: TEXT_SECONDARY }}>{data.about}</p>
              <div className="grid grid-cols-2 gap-4">
                {[{ icon: GraduationCap, label: "Certified Tutors" }, { icon: Lightbulb, label: "Personalized Plans" }, { icon: Star, label: "5-Star Rated" }, { icon: ChartLineUp, label: "Proven Results" }].map(b => (
      ...
      <section id="gallery" className="relative z-10 py-24 md:py-32" style={{ background: "#ffffff" }}>
        <div className="max-w-6xl mx-auto px-6">
          <SectionHeader badge="Learning Environment" title="Our Learning Center" accent={ACCENT} />
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {galleryImages.map((src: string, i: number) => {
              const titles = ["One-on-One Sessions", "Group Study", "Test Prep", "Online Tutoring"];
              return (
      ...
      <section className="relative z-10 py-24 md:py-32" style={{ background: BG }}>
        <div className="max-w-6xl mx-auto px-6">
          <SectionHeader badge="Success Stories" title="What Parents & Students Say" accent={ACCENT} />
      ...
      <section className="relative z-10 py-20 overflow-hidden" style={{ background: ACCENT }}>
        <div className="max-w-4xl mx-auto px-6 text-center">
          <GraduationCap size={48} weight="fill" className="mx-auto mb-6 text-white/80" />
          <h2 className="text-3xl md:text-5xl font-black tracking-tight text-white mb-4">Ready to Boost Your Grades?</h2>
          <p className="text-lg text-white/90 mb-8 max-w-xl mx-auto">Schedule a free consultation and discover how personalized tutoring can transform your academic performance.</p>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight mb-6" style={{ color: TEXT_PRIMARY }}>Schedule a Free Consultation</h2>
              <p className="leading-relaxed mb-8" style={{ color: TEXT_SECONDARY }}>Contact {data.businessName} to get started. We will match your student with the perfect tutor.</p>
            ...
            <Card className="p-8">
              <h3 className="text-xl font-semibold mb-6" style={{ color: TEXT_PRIMARY }}>Get Started Today</h3>
              <form className="space-y-4" onSubmit={e => e.preventDefault()}>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-sm mb-1.5" style={{ color: TEXT_SECONDARY }}>Parent Name</label><input type="text" className="w-full px-4 py-3 rounded-xl border text-sm focus:outline-none" style={{ borderColor: CARD_BORDER }} placeholder="John" /></div>
                  <div><label className="block text-sm mb-1.5" style={{ color: TEXT_SECONDARY }}>Student Name</label><input type="text" className="w-full px-4 py-3 rounded-xl border text-sm focus:outline-none" style={{ borderColor: CARD_BORDER }} placeholder="Emma" /></div>
                </div>
                ...
                <div><label className="block text-sm mb-1.5" style={{ color: TEXT_SECONDARY }}>Grade Level</label><input type="text" className="w-full px-4 py-3 rounded-xl border text-sm focus:outline-none" style={{ borderColor: CARD_BORDER }} placeholder="e.g. 8th Grade, High School, College" /></div>
                <div><label className="block text-sm mb-1.5" style={{ color: TEXT_SECONDARY }}>Subject Needed</label>
                ...
                <div><label className="block text-sm mb-1.5" style={{ color: TEXT_SECONDARY }}>Tell Us More</label><textarea rows={3} className="w-full px-4 py-3 rounded-xl border text-sm focus:outline-none resize-none" style={{ borderColor: CARD_BORDER }} placeholder="Describe your student's needs and goals..." /></div>
                <MagneticButton className="w-full py-4 rounded-xl text-base font-semibold text-white flex items-center justify-center gap-2 cursor-pointer" style={{ background: ACCENT } as React.CSSProperties}>Request Free Consultation <ArrowRight size={18} weight="bold" /></MagneticButton>
              </form>
            </Card>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          <ShimmerBorder accent={ACCENT}>
            <div className="p-8 md:p-12">
              ...
              <h2 className="text-2xl md:text-4xl font-extrabold mb-4" style={{ color: TEXT_PRIMARY }}>Our Results Guarantee</h2>
              <p className="leading-relaxed max-w-2xl mx-auto text-lg" style={{ color: TEXT_SECONDARY }}>{data.businessName} is committed to your student's success. If you are not satisfied after the first session, we will make it right.</p>
              <div className="flex flex-wrap justify-center gap-4 mt-8">
                {["Certified Tutors", "Free Consultation", "Flexible Scheduling", "Satisfaction Guaranteed"].map(item => (
      ...
      <footer className="relative z-10 border-t py-10" style={{ borderColor: CARD_BORDER, background: "#ffffff" }}>
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-3"><GraduationCap size={22} weight="fill" style={{ color: ACCENT }} /><span className="text-lg font-bold" style={{ color: TEXT_PRIMARY }}>{data.businessName}</span></div>
            ...
            <div>
              ...
              <div className="space-y-2">{["Services", "About", "Gallery", "Contact"].map(l => <a key={l} href={`#${l.toLowerCase()}`} className="block text-sm hover:text-amber-700 transition-colors" style={{ color: TEXT_MUTED }}>{l}</a>)}</div>
            </div>
            <div>
              ...
              <div className="space-y-2 text-sm" style={{ color: TEXT_MUTED }}>
                ...
                {data.socialLinks && Object.entries(data.socialLinks).map(([p, u]) => <a key={p} href={u as string} target="_blank" rel="noopener noreferrer" className="block hover:text-amber-700 capitalize">{p}</a>)}
              </div>
            </div>
          </div>
          <div className="border-t pt-6 flex flex-col md:flex-row items-center justify-between gap-4" style={{ borderColor: CARD_BORDER }}>
            <div className="flex items-center gap-2 text-sm" style={{ color: TEXT_MUTED }}><GraduationCap size={14} weight="fill" style={{ color: ACCENT }} /><span>{data.businessName} &copy; {new Date().getFullYear()}</span></div>
