...
const ICON_MAP: Record<string, any> = {
  install: Garage, repair: Wrench, spring: GearSix, opener: Lightning,
  maintain: GearSix, commercial: Buildings, residential: HouseLine,
  emergency: Warning, panel: Garage, cable: Wrench, track: Wrench,
  remote: Lightning, keypad: Lightning, insul: HouseLine, gate: Garage,
};
...
function GridPattern({ opacity = 0.04, accent }: { opacity?: number; accent: string }) {
  const pid = `garageGridV2-${accent.replace("#", "")}`;
  return (
    <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{ opacity }}>
      <defs>
        <pattern id={pid} width="50" height="50" patternUnits="userSpaceOnUse">
          <path d="M 50 0 L 0 0 0 50" fill="none" stroke={accent} strokeWidth="0.3" />
          <rect x="20" y="20" width="10" height="10" fill="none" stroke={accent} strokeWidth="0.2" opacity="0.5" />
        </pattern>
      </defs>
...
function ShimmerBorder({ children, className = "", accent }: { children: React.ReactNode; className?: string; accent: string }) {
  return (
    <div className={`relative rounded-2xl p-[1px] overflow-hidden ${className}`}>
      <motion.div className="absolute inset-0 rounded-2xl" style={{ background: `conic-gradient(from 0deg, transparent, ${accent}, transparent, #60a5fa, transparent)`, willChange: "transform" }} animate={{ rotate: [0, 360] }} transition={{ duration: 4, repeat: Infinity, ease: "linear" }} />
...
/* ═══════════════════════════════════════════════════════════════
   MAIN COMPONENT
   ═══════════════════════════════════════════════════════════════ */
export default function V2GarageDoorPreview({ data }: { data: GeneratedSiteData }) {
  ...
  const processSteps = [
    { step: "01", title: "Call or Book", desc: `Contact ${data.businessName} by phone or online. We offer same-day service for most repairs.` },
    { step: "02", title: "Inspection", desc: "Our certified technician inspects your garage door system and diagnoses the issue on the spot." },
    { step: "03", title: "Expert Repair", desc: "We carry most parts on our trucks for fast, same-visit repairs using quality, warrantied components." },
    { step: "04", title: "Quality Check", desc: "We test every component, ensure smooth operation, and review the work with you before we leave." },
  ];

  const faqs = [
    { q: `What garage door services does ${data.businessName} offer?`, a: `We provide ${data.services.slice(0, 3).map(s => s.name).join(", ")}, and more. Whether it's a broken spring, a new opener, or a full door replacement, we handle it all.` },
    { q: "How long does a garage door repair take?", a: "Most repairs are completed in 1-2 hours during a single visit. Our trucks are stocked with common parts so we can fix most issues on the spot without a return trip." },
    { q: "Do you offer emergency garage door service?", a: `Yes! ${data.businessName} offers emergency service for situations like a door stuck open, broken springs, or doors off the track. Call us anytime for fast response.` },
    { q: "How often should I service my garage door?", a: "We recommend annual maintenance to keep your garage door operating safely and smoothly. Regular service extends the life of springs, cables, and openers, and prevents costly breakdowns." },
  ];
  ...
  return (
    <main className="relative min-h-[100dvh] overflow-x-hidden" style={{ background: BG, color: TEXT_PRIMARY }}>
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6 py-4">
          <div className="flex items-center justify-between px-4 md:px-6 py-3 rounded-2xl bg-white/80 backdrop-blur-lg border shadow-sm" style={{ borderColor: CARD_BORDER }}>
            ...
            <div className="flex items-center gap-3">
              <MagneticButton className="px-4 md:px-5 py-2 rounded-full text-sm font-semibold text-white cursor-pointer" style={{ background: ACCENT } as React.CSSProperties}>Get a Quote</MagneticButton>
      ...
      <section className="relative min-h-[100dvh] flex items-center pt-24 z-10 overflow-hidden">
        ...
        <GridPattern opacity={0.03} accent={ACCENT} />
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 md:px-6 w-full grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          <div className="space-y-8">
            <div>
              <p className="text-sm uppercase tracking-widest mb-4 font-semibold" style={{ color: ACCENT }}>Professional Garage Door Service</p>
            ...
            <div className="flex flex-wrap gap-4">
              <MagneticButton className="px-8 py-4 rounded-full text-base font-semibold text-white flex items-center gap-2 cursor-pointer shadow-lg" style={{ background: ACCENT } as React.CSSProperties}>Schedule Service <ArrowRight size={18} weight="bold" /></MagneticButton>
            ...
            <div className="flex flex-wrap gap-6 text-sm" style={{ color: TEXT_SECONDARY }}>
              ...
              <span className="flex items-center gap-2"><Lightning size={16} weight="duotone" style={{ color: ACCENT }} /> Same-Day Service</span>
            </div>
          </div>
          <div className="hidden md:block relative">
            <div className="relative rounded-2xl overflow-hidden border shadow-xl" style={{ borderColor: CARD_BORDER }}>
              <img src={heroImage} alt={`${data.businessName} garage door`} className="w-full h-[500px] object-cover" />
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {data.stats.map((stat, i) => {
              const icons = [Garage, Wrench, Star, ShieldCheck];
      ...
      <section id="services" className="relative z-10 py-24 md:py-32 overflow-hidden" style={{ background: BG }}>
        <GridPattern opacity={0.02} accent={ACCENT} />
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          <SectionHeader badge="Our Services" title="Garage Door Solutions" subtitle={`${data.businessName} provides expert repair, installation, and maintenance for all garage door systems.`} accent={ACCENT} />
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <div>
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight mb-6" style={{ color: TEXT_PRIMARY }}>Your Garage Door Experts</h2>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { icon: GearSix, label: "All Brands Serviced" },
                ].map((b) => (
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {galleryImages.map((src, i) => {
              const titles = ["New Door Installation", "Spring Replacement", "Opener Upgrade", "Panel Repair"];
              return (
      ...
      <section className="relative z-10 py-20 overflow-hidden" style={{ background: ACCENT }}>
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
          <Garage size={48} weight="fill" className="mx-auto mb-6 text-white/80" />
          <h2 className="text-3xl md:text-5xl font-black tracking-tight text-white mb-4">Garage Door Emergency?</h2>
          <p className="text-lg text-white/90 mb-8 max-w-xl mx-auto">Broken spring, stuck door, or malfunctioning opener? We provide fast, reliable emergency service.</p>
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight mb-6" style={{ color: TEXT_PRIMARY }}>Get a Free Quote</h2>
              <p className="leading-relaxed mb-8" style={{ color: TEXT_SECONDARY }}>Need garage door repair or installation? Contact {data.businessName} for a free, no-obligation quote.</p>
            ...
            <Card className="p-8">
              <h3 className="text-xl font-semibold mb-6" style={{ color: TEXT_PRIMARY }}>Request a Free Quote</h3>
              <form className="space-y-4" onSubmit={(e) => e.preventDefault()}>
                ...
                <div><label className="block text-sm mb-1.5" style={{ color: TEXT_SECONDARY }}>Details</label><textarea rows={4} className="w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 resize-none" style={{ borderColor: CARD_BORDER, color: TEXT_PRIMARY }} placeholder="Describe your garage door issue..." /></div>
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 text-center">
          <ShimmerBorder accent={ACCENT}>
            <div className="p-8 md:p-12">
              ...
              <p className="leading-relaxed max-w-2xl mx-auto text-lg" style={{ color: TEXT_SECONDARY }}>{data.businessName} backs every repair and installation with a comprehensive warranty. Your satisfaction is guaranteed.</p>
              <div className="flex flex-wrap justify-center gap-4 mt-8">
                {["Licensed & Insured", "Parts Warranty", "Same-Day Service", "Free Estimates"].map((item) => (
      ...
      <footer className="relative z-10 border-t py-10 overflow-hidden" style={{ borderColor: CARD_BORDER, background: "#ffffff" }}>
        <div className="mx-auto max-w-6xl px-6 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-3"><Garage size={22} weight="fill" style={{ color: ACCENT }} /><span className="text-lg font-bold" style={{ color: TEXT_PRIMARY }}>{data.businessName}</span></div>
            ...
            <div>
              ...
              <div className="space-y-2">{["Services", "About", "Projects", "Contact"].map((l) => <a key={l} href={`#${l.toLowerCase()}`} className="block text-sm hover:text-blue-600 transition-colors" style={{ color: TEXT_MUTED }}>{l}</a>)}</div>
            </div>
            <div>
              ...
              <div className="space-y-2 text-sm" style={{ color: TEXT_MUTED }}>
                ...
                {data.socialLinks && Object.entries(data.socialLinks).map(([p, u]) => <a key={p} href={u} target="_blank" rel="noopener noreferrer" className="block hover:text-blue-600 transition-colors capitalize">{p}</a>)}
              </div>
            </div>
          </div>
          <div className="border-t pt-6 flex flex-col md:flex-row items-center justify-between gap-4" style={{ borderColor: CARD_BORDER }}>
            <div className="flex items-center gap-2 text-sm" style={{ color: TEXT_MUTED }}><Garage size={14} weight="fill" style={{ color: ACCENT }} /><span>{data.businessName} &copy; {new Date().getFullYear()}</span></div>
