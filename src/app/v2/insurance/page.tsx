...
function GlassCard({ children, className = "", glow, style, id, onClick, href }: { children: React.ReactNode; className?: string; glow?: string; style?: React.CSSProperties; id?: string; onClick?: () => void; href?: string }) {
  ...
  if (href) {
    return (
      <a href={href} id={id} onClick={onClick} className={`relative rounded-2xl border border-white/15 bg-white/[0.07] backdrop-blur-md ${className}`} style={mergedStyle}>
        {children}
      </a>
    );
  }
  return (
    <div id={id} onClick={onClick} className={`relative rounded-2xl border border-white/15 bg-white/[0.07] backdrop-blur-md ${className}`} style={mergedStyle}>
      {children}
    </div>
  );
}
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="border-t border-white/8 pt-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-slate-600 text-xs flex items-center gap-1.5">
            ...
            <a href="https://bluejayportfolio.com/audit" target="_blank" rel="noopener noreferrer" className="hover:text-slate-400 transition-colors underline">BlueJays</a>{" "}— get your free site audit
          </p>
        </div>
      </div>
    </footer>
