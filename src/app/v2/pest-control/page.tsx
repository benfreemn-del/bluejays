...
function ShieldHeroIcon() {
  return (
    <motion.div className="relative flex items-center justify-center" animate={{ rotateY: [0, 360] }} transition={{ duration: 14, repeat: Infinity, ease: "linear" }} style={{ perspective: 800, willChange: "transform" }}>
      <motion.div className="absolute inset-0 rounded-full" style={{ background: `radial-gradient(circle, ${ORANGE_GLOW} 0%, transparent 70%)`, filter: "blur(30px)" }} animate={{ scale: [1, 1.15, 1] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} />
      <svg viewBox="0 0 120 140" className="relative z-10 w-48 h-56 md:w-60 md:h-72" fill="none" xmlns="http://www.w3.org/2000/svg">
        <motion.path d="M60 10 L100 30 C100 80, 85 110, 60 130 C35 110, 20 80, 20 30Z" stroke={ORANGE} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="none" initial={{ pathLength: 0, opacity: 0 }} animate={{ pathLength: 1, opacity: 1 }} transition={{ duration: 2.5, ease: "easeInOut" }} />
        <motion.path d="M60 50 L50 65 L55 65 L45 85 L70 62 L62 62 L75 45Z" stroke={ORANGE_LIGHT} strokeWidth="1.5" fill="none" opacity={0.6} initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2, delay: 1 }} />
        <motion.circle cx="92" cy="20" r="3" fill={ORANGE_LIGHT} animate={{ opacity: [0.3, 1, 0.3], scale: [0.8, 1.2, 0.8] }} transition={{ duration: 2, repeat: Infinity }} />
      </svg>
    </motion.div>
  );
}
