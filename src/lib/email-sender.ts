...
import { assertEmailAllowed, type EmailCategory } from "./email-guard";
...
export async function sendEmail(
  ...
  options?: {
    ...
    /**
     * Guard category. Everything through sendEmail() is marketing /
     * lifecycle by default (cold outreach, audit follow-ups, NPS,
     * referral asks, renewals, win-loss, welcome series). Pass
     * "transactional" only for sends a human is actively waiting on.
     */
    category?: EmailCategory;
  },
): Promise<SentEmail> {
  // GLOBAL KILL SWITCH — see src/lib/email-guard.ts for modes + defaults.
  // Throws so the caller's existing try/catch takes the failure path
  // (no `*_sent_at` flag marked → automatic resume when the pause lifts).
  assertEmailAllowed(options?.category ?? "marketing", { to, subject });
