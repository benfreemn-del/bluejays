...
select * from (values
  ('olympic-inspections', 'ben', 'build', 'high',
   ...
   'Buy a dedicated Twilio number for olympicinspect.com and add it as OIT_TWILIO_NUMBER in Vercel env. Until this lands, the funnel runner skips SMS sends entirely (warm follow-ups go email-only). Number lives in src/lib/client-funnels/registry.ts under the OIT entry.'),
  ('olympic-inspections', 'ben', 'build', 'high',
   'Set up SendGrid DKIM on olympicinspect.com',
  ...
  ('olympic-inspections', 'client', 'asset', 'medium',
   'Luke: send 3 real testimonials to replace placeholders',
  ...
  ('olympic-inspections', 'client', 'asset', 'low',
   'Luke: send 1 hero photo (truck on-site, lab equipment, or owner)',
