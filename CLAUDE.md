...
| `meyer-electric` | ✓ (stats-only) | ✗ | ✗ | Kyle Meyer · Sequim WA · custom-tier bespoke · Live owner stats backend at `/clients/meyer-electric/stats` (password from `MEYER_STATS_PASSWORD` env var — repo is public, NEVER hardcode gate passwords; fails closed when unset) shipped 2026-07-15 — first-party page-view beacon (per-hostname: sequimelectrician.com / sequimelectric.com / bluejayportfolio.com path) + quote requests from `contact_form_submissions` + domains from `managed_domains`. **Footer-credit EXCEPTION (Ben, 2026-07-15):** the "Built by BlueJays" credit is REMOVED from this showcase — footer carries only a subtle lock icon linking to the stats page. Mock portal-demo (1212) stays live for sales demos, untouched. |
...
- **FROM_EMAIL must be hardcoded in EVERY SendGrid-calling route, not just email-sender.ts (Rule 16, strengthened 2026-05-12 as Rule 67, FROM-address swapped 2026-05-19)** — the Vercel FROM_EMAIL env var was set to an invalid value causing SendGrid `400 "Invalid from email address"` rejections. **Current locked value is `alerts@bluejayportfolio.com`** — an address on the SendGrid-authenticated `bluejayportfolio.com` domain (no separate single-sender verification needed; the domain auth covers it). Every SendGrid payload MUST also include `reply_to: { email: "bluejaycontactme@gmail.com", name: "BlueJays" }` so customer replies land in Ben's Gmail. **Why the swap landed 2026-05-19:** the prior value `bluejaycontactme@gmail.com` was DMARC-misaligned (From: `@gmail.com` sent through SendGrid SMTP, not Google's servers) and silently spam-quarantined OR hard-dropped by Gmail's strict DMARC policy on @gmail.com From-addresses. SendGrid Activity Feed showed "Delivered" status (= accepted by destination MX) for 15 OIT booking emails on the morning of 2026-05-19, but Ben confirmed NONE landed in inbox or spam folder. The new `alerts@bluejayportfolio.com` sends DMARC-aligned via the authenticated `bluejayportfolio.com` domain. **Migration complete 2026-05-19 (commit followup):** all 13 callsites migrated to `alerts@bluejayportfolio.com` with `reply_to: bluejaycontactme@gmail.com` added to every SendGrid payload — `src/lib/alerts.ts` (both `sendOwnerEmail` and `sendEmailTo`), `src/app/api/contact-form/[id]/route.ts`, `src/app/api/inquire/[code]/route.ts`, `src/app/api/webhooks/stripe/route.ts` (both payload sites), `src/app/api/leads/submit/route.ts`, `src/app/api/winback/route.ts`, `src/app/api/change-requests/route.ts`, `src/app/api/cron/agency-nurture/route.ts` (also updated List-Unsubscribe mailto + reply_to to point at the real-inbox Gmail), `src/app/api/cron/weekly-work-log-digest/route.ts`, `src/app/api/schedule/book/[id]/route.ts`, `src/app/api/review/submit/route.ts`, `src/app/api/clients/mt-view-landscaping/contact/route.ts` (kept conditional visitor-email reply_to override, added fallback default), `src/app/api/nps/feedback/[code]/route.ts` (`INBOX_EMAIL` is a recipient not FROM, hardcoded to remove the misleading env-var lookup), `src/app/client/[id]/page.tsx` (`CONTACT_EMAIL` display string hardcoded). Audit grep returns clean as of 2026-05-19. **Audit pattern when adding any new email-sending code:** run `grep -rn "process.env.FROM_EMAIL" src/` (must return ZERO matches), AND `grep -rn 'const FROM_EMAIL' src/` (every match must be `= "alerts@bluejayportfolio.com"`), AND every SendGrid payload with `from: { email: FROM_EMAIL` must have an adjacent `reply_to: { email: "bluejaycontactme@gmail.com"`. **After any SendGrid billing suspension + reactivation:** re-verify domain authentication for `bluejayportfolio.com` in the SendGrid dashboard before relying on any send. **Silent-failure debug pattern (CRITICAL):** SendGrid 400 rejections do NOT appear in the SendGrid Activity Feed — only the per-invocation Vercel function logs surface them. SendGrid 200 + DMARC-spam-filter ALSO looks healthy from SendGrid's side — diagnose by checking the Activity Feed `status` field AND the recipient's actual inbox (including spam folder). Every SendGrid call must log on EVERY failure branch: missing key (`console.error`), fetch throw (`try/catch`), `!sgRes.ok` (log status + body). Reference implementation: `src/lib/alerts.ts` after the 2026-05-19 update. Full memory entry: `feedback_sendgrid_from_email_hardcode.md` in user memory.
