...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Intercepts all forms posting to Web3Forms, shows inline
  // success/error messages without navigating away from the page.
  // ============================================================
  document.querySelectorAll('form[action*="web3forms.com"]').forEach((form) => {
    form.addEventListener('submit', async (e) => {
      const accessKey = (form.querySelector('input[name="access_key"]') || {}).value || '';
      if (!accessKey || accessKey.includes('[[WEB3FORMS_')) {
        e.preventDefault();
        showStatus(form, 'Form not yet configured. Please contact info@lcautism.org directly.', 'warn');
        return;
      }
      ...
      try {
        ...
        if (data.success) {
          ...
          showStatus(form, data.message || 'Something went wrong. Please try again — or email info@lcautism.org directly.', 'err');
        }
      } catch (err) {
